    ...
    <>
      ...
      >
        ...
        <div className="w-full h-full flex justify-end items-center" onClick={() => handleMenu()}>
          <IoMenuOutline color={THEME.text} size={35} />
        </div>
      </div>

      <div className={`z-50 fixed top-0 right-0 w-full h-full transition-[backdrop-filter] duration-600 md:hidden ${isOpen ? 'translate-x-0 backdrop-blur-xs' : 'translate-x-full'}`} onClick={() => handleMenu()} />
      ...
      >
        <div className="h-20 flex flex-col items-end justify-center p-6" onClick={() => handleMenu()}>
          {isOpen && <IoCloseOutline color={THEME.text} size={35} />}
        </div>
